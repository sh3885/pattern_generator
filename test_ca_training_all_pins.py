...
for pin in ca_pins:
    ...
    print(f'Raw pattern: {raw_frames} frames (48 training)')
...
for pin in ca_pins:
    training = pattern_generator.generate_ca_training_pattern(pin, '00111100', clock_toggle=True, num_frames=48)
    serdes_pattern = pattern_generator.pattern_to_serdes_16to1(training, padding_ck_toggle=True, padding_ck_value=0)
    ca_trn_pat_list.append(f"0x{serdes_pattern.upper()}")
    
    final_misr = pattern_generator.get_aword_misr(training)
